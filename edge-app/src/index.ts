...
app.get('/api/nodes/latest', async (c) => {
  const backend = c.env.API_BACKEND_URL || 'https://tunnel.example.com'
  try {
...
app.get('/', (c) => {
  const supabaseUrl = c.env.SUPABASE_URL || 'https://service.example.com'
  const supabaseKey = c.env.SUPABASE_ANON_KEY || 'replace-with-a-publishable-key'
...
export default {
  ...
  async scheduled(event: any, env: Bindings, ctx: any) {
    const url = `${env.API_BACKEND_URL || 'https://tunnel.example.com'}/api/maintenance/prune`;
    try {
