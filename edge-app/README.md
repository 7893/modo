...
- **Live URL**: [https://service.example.com](https://service.example.com)
- **Framework**: [Hono](https://service.example.com) on Cloudflare Workers
...
Cloudflare Workers Edge (modo.53.workers.dev)
      ...
      ├─ GET /api/* (Hono Router) ────────> Reverse proxy to https://tunnel.example.com
...
2. **Edge Proxy API Router (`src/index.ts`)**:
   ...
   - Routes requests to the private API gateway (`https://tunnel.example.com`) through the Zero-Trust Cloudflare Tunnel:
...
| `vars.API_BACKEND_URL` | Upstream private API gateway via Cloudflare Tunnel | `https://tunnel.example.com` |
| `vars.SUPABASE_URL` | Supabase Authentication project URL | `https://service.example.com` |
