...
CPU_WARMUP_SECONDS = max(0.0, float(os.getenv("CPU_WARMUP_SECONDS", "1")))
...
# Target nodes are loaded by main(), keeping module imports free of config I/O.
...
def scrape_all_nodes() -> list:
    ...
    if not TARGET_NODES:
        logger.warning("No target nodes configured; skipping scrape")
        return []
...
def main():
    ...
    logger.info("Warming CPU counters before the first persisted sample...")
    scrape_all_nodes()
    if CPU_WARMUP_SECONDS:
        time.sleep(CPU_WARMUP_SECONDS)
